...
class ChirurgienController extends Controller
{
    ...
    public function AbservationMedicaleCreate(Patient $patient)
    {
        ...
        $data = Cache::remember($cacheKey, 600, function () use ($patient) {
            return [
                'observation_medicales' => ObservationMedicale::select(['id', 'observation', 'date', 'anesthesiste', 'patient_id', 'user_id'])
                    ->with('patient:id,name,prenom', 'user:id,name')
                    ->where('patient_id', $patient->id)
                    ->paginate(15),
                'soins_infirmiers' => SoinsInfirmier::select(['id', 'soin', 'date', 'patient_id', 'user_id'])
                    ->with('patient:id,name,prenom', 'user:id,name')
                    ->where('patient_id', $patient->id)
                    ->paginate(15)
            ];
        ...
        $anesthesistes = Cache::tags(['users'])->remember('anesthesistes_cache', 1800, function () {
            return User::whereIn('name', ['TENKE', 'SANDJON'])
                ->select(['id', 'name'])
                ->get();
        });

        $users = Cache::tags(['users'])->remember('users_role_2_obs', 1800, function () {
            return User::where('role_id', 2)
                ->select(['id', 'name'])
                ->get();
        });

        $patients_externes = Cache::tags(['clients'])->remember('clients_all', 600, function () {
