                ...
                <div class="row">
                    <div class="col-md-12 col-lg-10 offset-md-1">
                        <table class="table">
                            ...
                            <tbody>
                                {{-- On vérifie si la collection n'est pas vide --}}
                                @if (isset($cartItems) && $cartItems->count() > 0)
                                    @foreach ($cartItems as $item)
                                        <tr data-product-id="{{ $item->produit_id }}">
                                            <td class="col-md-8 col-lg-6">
                                                <div class="media-body">
                                                    {{-- Accès via la relation 'produit' du modèle CartItem --}}
                                            ...
                                            <td class="col-md-1 col-lg-1" style="text-align: center">
                                                <input type="number" class="form-control quantity-input" min="1"
                                                    value="{{ $item->quantite }}" data-product-id="{{ $item->produit_id }}"
                                                    data-old-qty="{{ $item->quantite }}">
                                            </td>
                                            <td class="col-md-1 col-lg-1 text-center">
                                                <strong>{{ $item->prix_unitaire }}</strong>
                                            </td>
                                            <td class="col-md-1 col-lg-1 text-center item-total">
                                                {{-- Calcul du total pour la ligne --}}
                                                <strong>{{ $item->quantite * $item->prix_unitaire }}</strong>
                                            </td>

                                            {{-- Actions --}}
                                            <td>
                                                <a href="{{ route('facturation.reduire', $item->produit_id) }}"
                                                    class="btn btn-primary">
                                                    <i class="fas fa-minus"></i>
                                                </a>
                                            </td>
                                            <td>
                                                <a href="{{ route('pharmaceutique.cart', $item->produit_id) }}"
                                                    class="btn btn-success">
                                                    <i class="fas fa-plus-square"></i>
                                                </a>
                                            </td>
                                            <td>
                                                <a href="{{ route('facturation.supprimer', $item->produit_id) }}"
                                ...
                                @else
                                    <tr>
                                        <td colspan="7" class="text-center">Le panier est vide.</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
                            ...
                            <div class="form-group">
                                ...
                                <select name="patient" id="patient" class="form-control col-md-5 mb-2" required>
                                    ...
                                    @foreach ($patient as $patients)
                                        <option value="{{ $patients->name }}">{{ $patients->name }}
                                            {{ $patients->prenom }}</option>
                                    @endforeach
                                </select>
                            </div>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.querySelectorAll('.quantity-input').forEach(input => {
                    input.addEventListener('change', function() {
                        ...
                        console.log(
                            `🔢 Changement quantité produit [${productId}]: ${oldQty} -> ${newQty}`);
